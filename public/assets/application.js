...
var game = {
	...
	mapListener: function(){
		//alert(1);
		setTimeout(function(){
			//var map1 = new google.maps.Map(document.getElementById("map_canvas"), map);
		...
		$.getJSON('http://engine.souplus.com/api/trips.json?tab=' + id, function(response) {
			  var data = response.data,
				...
				//if (GBrowserIsCompatible()) {
					...
					var latlog = document.getElementById("end").value,
						lat = '',
						log = '';
					if(latlog && latlog != ""){
						lat = latlog.split(',')[0].trim();
						lat = lat.substr(1,lat.length);
						log = latlog.split(',')[1].trim();
					...
					google.maps.event.addDomListener(window, 'load', game.mapListener());
