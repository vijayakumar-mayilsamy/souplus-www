...
var game = {
	...
	viewGame: function(game_id){
		...
		$.getJSON('http://engine.souplus.com/api/games/profile.json?game_id='+game_id.trim()+'&token='+ this.token(), function(response) {
			 // alert(response.data);	
			  var data = response.data,
				  d = data, 
				  id = d.id,
			  ...
			  $('#game_meta').html('<div class="span5"><h2 style="color:#B94A48;">'+w_name+'</h2><img src="/assets/testing/fea_i2.jpg" style="width:100px;height:150px;" /></div><div class="span2" style="margin:0;padding:0;"><h2>Vs</h2></div><div class="span5"><h2 style="color:#58c;">'+l_name+'</h2><img src="/assets/testing/fea_i1.jpg" style="width:100px;height:150px;" /></div>');	
...
var userBar = {	
	refresh: function(score_update, appetite_update){
		...
		if (token!=null){
			...
			$('#user-bar').empty();
			var signin = $('<a id="signin-link"  class="btn btn-primary" onclick="javascript:$(&quot;#modal_overlay&quot;).show();$(&quot;#signin-modal&quot;).modal(&quot;show&quot;);"> Sign In </a>');
			$('<li> </li>').append(signin).appendTo('#user-bar');
			var signup = $('<a href="/signup/to-be-verified" id="signup-link" class="btn btn-primary"> Sign Up </a>');
			$('<li> </li>').append(signup).appendTo('#user-bar');
...
var user = {
	signin_cancel: function(){
		$('#signin-modal').slideUp().remove();
		$("#modal_overlay").hide();
	}		
};
var autoRefresh = null,  
...
var game = {
	...
	home: function(){
		$.getJSON('http://engine.souplus.com/api/games.json', function(response) {
				  var data = response.data;
				  //alert(game.token());
					game.featuredGame(data);
					game.allGames(data);
...
$(document).ready(function(){
	$('body').append('<div id="modal_overlay" style="position:absolute;left:0;top:0;display:none;background:#000;width:100%;height:1000px;opacity:0.5;filter:alpha(opacity=100);"></div>');
	userBar.refresh(false, false);	
	//enableFeedbackForm();
});
